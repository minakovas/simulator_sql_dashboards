/*
Расчёт Retention для двух рекламных кампаний
*/
...
SELECT CONCAT('Кампания № ', ads_campaign) AS ads_campaign,
       CONCAT('Кампания № ', ads_campaign, ', ', start_date) AS ads_campaign_and_start_date,
       start_date,
       day_number,
       MAX(active_users) OVER (PARTITION BY ads_campaign, start_date) AS first_day_users,
       active_users,
       ROUND((active_users::DECIMAL / MAX(active_users) OVER (PARTITION BY ads_campaign, start_date)) * 100, 2) AS retention
FROM (
    SELECT ads_campaign,
           date,
           start_date,
