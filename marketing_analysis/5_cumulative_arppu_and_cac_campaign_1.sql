/*
Расчёт накопительного ARPPU и CAC для двух рекламных кампаний
Суть расчёта накопительного ARPPU заключается в том, что для каждого дня число покупателей в когорте будет оставаться неизменным, а полученный от их заказов доход — расти. 
В результате накопительный ARPPU будет постепенно увеличиваться и в какой-то момент превысит сумму изначальных затрат на привлечение одного покупателя.
*/
...
-- Готовим основную таблицу, по которой будем все считать
main_table AS (
SELECT user_id,
       ads_campaign,
       date,
...
-- Основной расчет. Тут обратить внимание, что считаю CAC & paying_users и revenue по main_table независимо друг от друга,
-- а затем объединяю результаты и считаю arppu
result AS (
SELECT CONCAT('Кампания № ', ads_campaign) AS ads_campaign,
       CONCAT('День ', date - MIN(date) OVER (PARTITION BY ads_campaign)) AS day,
       ROUND(SUM(revenue / paying_users) OVER (PARTITION BY ads_campaign ORDER BY date), 2) AS cumulative_arppu,
       ROUND(cac, 2) AS cac
FROM (
    SELECT ads_campaign,
           date,
...
LEFT JOIN (
    SELECT ads_campaign,
           ...
           250000.0 / COUNT(DISTINCT user_id) AS cac
    FROM main_table
...
SELECT * FROM result WHERE ads_campaign = 'Кампания № 1'
